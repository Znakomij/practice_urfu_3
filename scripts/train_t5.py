...
SAVE_DIRECTORY = "./t5_model_0.9_kw"
...
print("Загружаем очищенный и подготовленный датасет...")
data = pd.read_csv(
...
valid_dataloader = DataLoader(dataset["validation"], batch_size=4, num_workers=9)
test_dataloader = DataLoader(dataset["test"], batch_size=4, num_workers=9)
...
print(f"Обученная модель сохрнена в каталоге {SAVE_DIRECTORY}")
